...
class ApifyTweetScraperClient:
    ...
    def _build_input(
        ...
        payload.setdefault("handles", handles)
        payload.setdefault("usernames", handles)
        payload.setdefault("maxItems", total_limit)
        payload.setdefault("tweetsDesired", payload.get("maxItems", total_limit))
        if since_map:
            payload.setdefault("sinceIds", {k: v for k, v in since_map.items() if v})
        return payload

    def _start_run(self, input_payload: Dict) -> Dict:
        url = (
            f"{self.base_url}/acts/{urllib.parse.quote(self.actor_id)}/runs"
            f"?token={urllib.parse.quote(self.token)}&waitForFinish={self.timeout_seconds}"
        )
    ...
    def _poll_run(self, run_id: Optional[str]) -> Dict:
        ...
        while time.time() < deadline:
            ...
            with urllib.request.urlopen(request, timeout=self.poll_interval + 5) as response:
                body = json.loads(response.read().decode("utf-8"))
    ...
    def _fetch_dataset_items(self, dataset_id: str) -> List[Dict]:
        url = (
            ...
            f"?token={urllib.parse.quote(self.token)}&format=json"
        )
